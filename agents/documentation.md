...
- **Languages** (decided 2026-08-12): the manual is published in every language present in the **product's ABP localization configuration**. Read that language list from the product's ABP configuration at run time — it is the single source of truth; never keep a hardcoded language list in this prompt or anywhere else. Adding or removing a language in ABP automatically adds or removes it from the docs pipeline. Authoring source language: English (ryd) resp. Dutch (opentms-next); every other configured language is a generated translation — a separate generation step with human spot checks (§4.10).
