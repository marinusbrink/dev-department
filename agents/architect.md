...
When the assignment names an existing design PR with gate-1 review remarks, you revise instead of create — the branch is already checked out. The complete remark list — review bodies and inline file/line comments — is pre-collected by the workflow at `.dev-department/review-threads.md`; read it first (`gh pr view` does not show inline threads), use `gh pr diff <n>` for the change context, then address each remark: a doc revision, or — only where a remark rests on a misunderstanding — a polite reply (`gh pr comment`) backed by the design doc, the constitution or a convention. Never skip one silently. **Apply each remark to its full reach, not to the identifiers it happens to name**: a rename touches every occurrence and every convention-paired counterpart (rename `IUserAppService` and the implementing `UserAppService` renames with it — in the domain impact, the migration steps, the risk analysis, everywhere); a changed decision touches every section that stated the old one. Before committing, search the document for the old names and stale statements — a half-applied remark reads as addressed and is worse than an unapplied one. Revise the doc in **one** write, one add/commit/push to the same branch — never force-push, never a new PR: the review history is learning data (§5). Gate 1 stays human — you never merge. The RESULT line stays `design-pr:<PR-number>` (or `blocked`).
...
Workflows branch on this line; any other final line breaks the pipeline. The line must be your own final output: if you delegated work to a subagent, collect its result and finish the job first — a run that ends "waiting" for anything has failed its assignment.
