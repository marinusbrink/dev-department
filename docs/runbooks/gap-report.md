...
- [ ] **Build-phase toolchain grants undecided**: the implementer/test-engineer duties "solution builds and tests pass locally" / "run everything" need product-specific build, test and package-manager commands (dotnet, node/yarn, test runners) that no allowlist grants yet — the agents can now write and push but not compile or run suites. Decide these deliberately at phase-3 activation, before enabling `dept-build.yml`/`dept-fix.yml`; candidate mechanism: a caller-configurable extra-allowed-tools input, since the reusable workflows are product-agnostic and only the product caller knows its toolchain.
