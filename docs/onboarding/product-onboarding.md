...
| **3 — weeks 2–3, after first designs** | `dept-build.yml` + `dept-fix.yml` | Full build pipeline and the gate-2 fix loop. **Before enabling: fill `extra-allowed-tools` in both callers with the product's CI toolchain, and `contract-refresh-command` in `dept-build.yml` with a repo script that regenerates the OpenAPI spec + typed client** (template comments show how; see opentms-next's `scripts/contract-refresh-ci.sh`) — without the first the agents can push but not build; without the second the frontend inherits a stale contract. **Start with one low-risk domain** (for ryd: Master Data) — pick the first PBIs accordingly; serialization per domain is the rule anyway (§10). |
